...
def wizard():
    ...
    try: # Collect information
        ...
        crack(passsel, passlist, website) # Start the attack
    except KeyboardInterrupt: # If user exits the program manually
        print(color.RED + '\n[!] '+color.WHITE + 'Process terminated by user. Exiting...')
        exit()
...
def crack(passsel, passlist, website):
    ...
    try: # Start the attack
        ...
        print(color.RED + '\n[!] '+color.WHITE + 'Process terminated by user. Exiting...')
        exit()
    except selenium.common.exceptions.NoSuchElementException: # If the password field gets hidden, that means either the password is found if you are IP banned
...
if options.website == None and options.passsel == None and options.passlist == None: # if no arguments are given
    wizard()
missing_args = ""
...
if missing_args != "": # If any (but not all) arguments are missing
    ...
    try: # Check if the page is accecible
        ...
        print(color.RED + '[X] ' + '\n[!]'+color.WHITE + 'Connection timed out')
        exit()
    crack(options.passsel, options.passlist, options.website) # Start the attack
